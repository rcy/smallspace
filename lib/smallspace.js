if (Meteor.isClient) {
  ...
  Deps.autorun(function() {
    ...
    if (spaceId) {
      ...
      Meteor.subscribe('todoLists', spaceId);
    }
  });
  ...
  Template.spaceListItem.helpers({
    image: function() {
      return this.image || 'http://www.gravatar.com/avatar/'+CryptoJS.MD5(this._id)+'.jpg?d=monsterid&s=200'
    },
  ...
  Template.linksTab.helpers({
    links: function() {
      return Links.find({spaceId: Session.get('currentSpace')}, { sort: {created: -1} });
    }
  });
  Template.link.helpers({
    ...
    inline: function() {
      var match = this.url.match(/youtube.com\/.*watch\?.*v=([-a-zA-Z0-9_]+)/)
      if (match) {
...
if (Meteor.isServer) {
  Meteor.publish('messages', function(spaceId) {
    return Messages.find({spaceId: spaceId}, {sort: {created: -1}, limit: 50});
  });
  Meteor.publish('links', function(spaceId) {
    return Links.find({spaceId: spaceId}, { sort: { created: -1 }, limit: 5 } );
  });
